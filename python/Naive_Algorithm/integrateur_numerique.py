...
def mettre_a_jour_position_et_vitesse(systeme):
    ...
    def position_suivante_un_objet(particule, dt):
        ...
        x_next = particule.position[0] + particule.vitesse[0] * dt + (1/2) * particule.force[0] * (dt**2)
        y_next = particule.position[1] + particule.vitesse[1] * dt + (1/2) * particule.force[1] * (dt**2)
        z_next = particule.position[2] + particule.vitesse[2] * dt + (1/2) * particule.force[2] * (dt**2)
        ...
        particule.vitesse  = np.array([vx_next, vy_next, vz_next])
